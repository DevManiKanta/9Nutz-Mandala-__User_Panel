...
export default function UserDashboard() {
  ...
  // ========== PAGINATION SETTINGS FOR FAVOURITES ==========
  const PAGE_SIZE = 5;
  ...
  // Reset pagination when favourites change or when opening the favourites tab
  useEffect(() => {
    setFavPage(1);
  }, [favorites, activeSection]);

  // Ensure current page is within bounds when favourites length changes
  useEffect(() => {
    const totalPages = Math.max(1, Math.ceil(favouriteRows.length / PAGE_SIZE));
    if (favPage > totalPages) {
      setFavPage(totalPages);
    }
  }, [favouriteRows.length, favPage]);

  // Slice rows for the current page
  const totalFavItems = favouriteRows.length;
  const totalFavPages = Math.max(1, Math.ceil(totalFavItems / PAGE_SIZE));
  const pagedFavouriteRows = favouriteRows.slice(
    (favPage - 1) * PAGE_SIZE,
    favPage * PAGE_SIZE
  );
                        ...
                        {totalFavPages > 1 && (
                          <div className="mt-4 flex items-center justify-between">
                            <div className="text-sm text-gray-600">
                              Showing{" "}
                              <span className="font-medium">{(favPage - 1) * PAGE_SIZE + 1}</span>{" "}
                              to{" "}
                              <span className="font-medium">
                                {Math.min(favPage * PAGE_SIZE, totalFavItems)}
                              </span>{" "}
                              of <span className="font-medium">{totalFavItems}</span>
                            </div>
                            ...
                            <div className="flex items-center space-x-2">
                              <button
                                ...
                                className={`px-3 py-1 rounded-lg border ${
                                  favPage === 1
                                    ...
                                    : "text-gray-700 border-gray-200 hover:bg-white"
                                } text-sm`}
                              ...
                              {/* simple numeric buttons for up to 7 pages; if many pages, show first/last + nearby */}
                              <div className="flex items-center space-x-1">
                                {Array.from({ length: totalFavPages }).map((_, idx) => {
                                  const pageNum = idx + 1;
                                  // show all if small, otherwise show window around current
                                  if (totalFavPages <= 7 || Math.abs(favPage - pageNum) <= 2 || pageNum === 1 || pageNum === totalFavPages) {
                                    return (
                                      <button
                                        key={pageNum}
                                        ...
                                        className={`px-2 py-1 rounded text-sm border ${
                                          favPage === pageNum
                                            ...
                                            : "border-gray-100 text-gray-700 hover:bg-white"
                                        }`}
                                        aria-current={favPage === pageNum ? "page" : undefined}
                                      >
                                        {pageNum}
                                      </button>
                                    );
                                  } else {
                                    // avoid rendering too many numbers: render ellipsis where appropriate
                                    const shouldShowLeftEllipsis = pageNum === 2 && favPage > 4;
                                    const shouldShowRightEllipsis = pageNum === totalFavPages - 1 && favPage < totalFavPages - 3;
                                    if (shouldShowLeftEllipsis || shouldShowRightEllipsis) {
                                      return (
                                        <span key={`ell-${pageNum}`} className="px-2 text-sm text-gray-400">
                                          …
                                        </span>
                                      );
                                    }
